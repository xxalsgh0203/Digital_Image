...
def main():
    ...
    output_image_name = outputDir + 'merged_image' + "_c_%s_a_%s_b_%s" % (column, alpha, beta) + ".jpg"
    ...
    output_image_name = outputDir + 'scaled_image' + "_c_%s_a_%s_b_%s" % (column, alpha, beta) + ".jpg"
    cv2.imwrite(output_image_name, scaled_image)

    centralized_pixels = operation_obj.centralize_pixel(merged_image, column=column)
    output_image_name = outputDir + 'centralized_image' + "_c_%s_a_%s_b_%s" % (column, alpha, beta) + ".jpg"
